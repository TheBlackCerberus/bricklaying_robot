...
def main():
    ...
    parser.add_argument(
        ...
        help="Wall configuration (stretcher_bond_wall, english_cross_bond_wall, flemish_bond_wall)"
    )
    parser.add_argument(
        "--algo", 
        default="naive_build",
        help="Build algorithm (naive_build)"
    )
    parser.add_argument(
        "--scale", 
        type=float, 
        default=0.4,
        help="Rendering scale factor (0.1-1.0, default: 0.4)"
    )
    parser.add_argument(
        "--debug", 
        action="store_true",
    ...
    for i, stride in enumerate(strides):
        print(f"  Stride {i+1}: ({stride.robot_position.x:.0f}, {stride.robot_position.y:.0f}) - {len(stride.bricks)} bricks")
    ...
    renderer = PygameRenderer(config, scale=args.scale, debug_mode=args.debug)
    
    print("Wall Builder")
    print("==========================================================")
    print(f"Wall size: {config['wall']['width']}mm × {config['wall']['height']}mm")
    print(f"Total bricks: {len(wall.bricks)}")
            ...
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_ESCAPE:
                    ...
                    if robot_mode and current_stride_index < len(strides):
                        ...
                        print(f"Robot moved to stride {current_stride_index + 1} position: ({robot.position.x:.0f}, {robot.position.y:.0f})")
                    ...
                    if not robot_mode:
                        ...
                        if current_brick_index < len(wall.bricks):
                            ...
                            built_count = sum(1 for b in wall.bricks if b.state == BrickState.BUILT)
                            progress = (built_count / len(wall.bricks) * 100)
                            print(f"Built brick {current_brick_index}/{len(wall.bricks)} ({progress:.1f}%)")
                        ...
                        if current_stride_index < len(strides):
                            ...
                            if current_brick_in_stride < len(stride.bricks):
                                ...
                                print(f"🤖 Built brick {built_in_stride}/{total_in_stride} in stride {current_stride_index + 1} ({progress:.1f}% total)")
                                
                                if current_brick_in_stride >= len(stride.bricks):
                                    print(f"✅ Stride {current_stride_index + 1} completed!")
                    ...
                    if current_stride_index < len(strides):
                        ...
                        built_count = sum(1 for b in wall.bricks if b.state == BrickState.BUILT)
                        progress = (built_count / len(wall.bricks) * 100)
                        print(f"🤖 Completed stride {current_stride_index + 1} ({len(stride.bricks)} bricks, {progress:.1f}% total)")
                    ...
                    remaining = len(wall.bricks) - sum(1 for b in wall.bricks if b.state == BrickState.BUILT)
                    if remaining > 0:
                        for brick in wall.bricks:
                            brick.state = BrickState.BUILT
                        current_brick_index = len(wall.bricks)
                        print(f"Built {remaining} remaining bricks - Wall completed! 🎉")
                ...
                renderer.screen = pygame.display.set_mode((renderer.window_width, renderer.window_height), pygame.RESIZABLE)
